...
- ✅ 納期 7 日前の警告表示
- ✅ ステータスのプルダウン検索・並び替え
- ✅ 開閉式の詳細検索パネル（既定は折りたたみ）：キーワード検索（件名・絵師名・X ID・メモ）、
...
- ✅ 依頼一覧カードはコンパクトな2行構成（タイトル+ステータス／絵師名+納期のみ）。詳細な日付はカードをタップした先の詳細画面で確認
...
**その他**
- ✅ デモモード（ログイン不要・メモリのみ・お知らせ・お問い合わせボタン付き。検索UX・お知らせ閲覧はCommissionAppと共通コンポーネント、お知らせは未読管理なしの読み取り専用）
- ✅ PWA 対応（ホーム画面追加。Androidではインストール操作、iOSではSafariの共有メニューから追加する方法を案内するバナーを表示）
...
`docs/handover.md` の「パフォーマンス改善」セクションを参照）。
...
- **外部キー列にはインデックスを張る**：PostgreSQLは外部キーに自動でインデックスを
  ...
  合わせて検討すること（`supabase/migrations/20260922000000_V1.2.3_add_performance_indexes.sql`
  が既存テーブルへの追加例）。
- **サードパーティスクリプトは`lazyOnload`を優先する**：GA・広告など計測系の
  外部スクリプトは、メインコンテンツの表示・操作可能化をブロックしないよう
  `next/script`の`strategy="lazyOnload"`を基本とすること。
...
- **本番とデモで表示・検索ロジックが同じ部分は共通化する**：`CommissionApp.tsx`
  （本番）と`DemoApp.tsx`（ログイン不要のデモ）は見た目・検索条件が同じである
  べきなのに、コードが別々にコピーされていたため機能追加のたびに片方だけ
  反映される問題があった。定数・フォーマッタ・UI部品（一覧カードの
  `CommissionListCard`を含む）は`components/CommissionShared.tsx`、
  検索・フィルタ・並び替えのロジックは`hooks/useCommissionSearch.ts`、
  検索バーのUIは`components/CommissionSearchBar.tsx`に集約している。
  新しい検索条件や見た目の変更は、この3ファイルを編集し、
  `CommissionApp.tsx`/`DemoApp.tsx`側は極力変更しないこと。
