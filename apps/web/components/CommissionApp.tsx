...
function CardThumbnail({ storagePath }: { storagePath: string }) {
  ...
  useEffect(() => {
    getSignedImageUrl(supabase, storagePath).then(setUrl).catch(() => { });
  }, [storagePath]);
  if (!url) return (
    <div style={{ width: 72, height: 72, borderRadius: 10, background: "#e5e7eb", flexShrink: 0 }} />
  );
  return (
    ...
    <div style={{ marginTop: 20 }}>
      ...
      {toast && (
        <div style={{
          ...
          background: "#7c3aed", color: "#fff", padding: "12px 24px", borderRadius: 12,
          fontSize: 14, fontWeight: 700, boxShadow: "0 4px 16px #0003", zIndex: 999
        }}>
          {toast}
        </div>
      )}
...
export default function CommissionApp() {
  ...
  // ---- 画像枚数集計 ----
  useEffect(() => {
    const total = commissions.reduce((sum, c) => sum + (c.images?.length ?? 0), 0);
    setImageCount(total);
  }, [commissions]);
  ...
  async function handleDeleteRequest() {
    ...
    try {
      await requestDeleteAccount(supabase);
  ...
  // ---- スクロールロック ----
  useEffect(() => {
    document.body.style.overflow = (showForm || !!detailId) ? "hidden" : "";
    ...
    <div style={{ minHeight: "100vh", background: "#faf8f5" }} onClick={() => setShowUserMenu(false)}>
      ...
      {showDeleteRequest && (
        <div style={{ position: "fixed", inset: 0, background: "#0007", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}
          ...
          <div style={{ background: "#fff", borderRadius: 20, padding: "32px", maxWidth: 380, width: "90%", boxShadow: "0 8px 48px #0004", textAlign: "center" }}
            onClick={e => e.stopPropagation()}>
            {deleteRequestDone ? (
              <>
                <div style={{ fontSize: 48, marginBottom: 12 }}>📨</div>
                <div style={{ fontWeight: 800, fontSize: 17, color: "#1a0a2e", marginBottom: 12 }}>申請を送信しました</div>
                <div style={{ fontSize: 13, color: "#666", lineHeight: 1.7, marginBottom: 24 }}>
                  管理者にメールで通知しました。<br />削除が完了するまで少しお待ちください。
                </div>
                <button onClick={() => { setShowDeleteRequest(false); setDeleteRequestDone(false); }}
                  style={{ background: "linear-gradient(135deg,#7c3aed,#4f46e5)", color: "#fff", border: "none", borderRadius: 10, padding: "11px 28px", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>
                  閉じる
                </button>
              </>
            ) : (
              <>
                ...
                <div style={{ fontWeight: 800, fontSize: 17, color: "#1a0a2e", marginBottom: 12 }}>アカウント削除を申請する</div>
                <div style={{ fontSize: 13, color: "#666", lineHeight: 1.7, marginBottom: 24 }}>
                  管理者にメールで削除申請を送ります。<br />削除されると<strong>すべてのデータが失われます。</strong>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={() => setShowDeleteRequest(false)} disabled={deleteRequesting}
                    style={{ flex: 1, background: "#f3f4f6", border: "none", borderRadius: 10, padding: "11px", fontWeight: 600, cursor: "pointer", fontSize: 14 }}>
                    キャンセル
                  </button>
                  <button onClick={handleDeleteRequest} disabled={deleteRequesting}
                    style={{ flex: 1, background: deleteRequesting ? "#fca5a5" : "#ef4444", color: "#fff", border: "none", borderRadius: 10, padding: "11px", fontWeight: 800, cursor: deleteRequesting ? "not-allowed" : "pointer", fontSize: 14 }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "#0006", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}
          ...
          <div style={{ background: "#fff", borderRadius: 20, padding: "32px", maxWidth: 360, width: "90%", boxShadow: "0 8px 48px #0004" }}
            ...
            <input
              ...
              placeholder="例: 山田太郎"
              maxLength={30}
              style={{ width: "100%", padding: "10px 13px", border: "1.5px solid #e5e7eb", borderRadius: 12, fontSize: 16, outline: "none", color: "#1a0a2e", background: "#faf8f5", boxSizing: "border-box", fontFamily: "inherit", marginBottom: 16 }}
            />
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={handleSaveName} disabled={!nameInput.trim()}
                style={{ flex: 2, background: nameInput.trim() ? "linear-gradient(135deg,#7c3aed,#4f46e5)" : "#c4b5fd", color: "#fff", border: "none", borderRadius: 10, padding: "11px", fontWeight: 800, cursor: nameInput.trim() ? "pointer" : "not-allowed", fontSize: 14 }}>
                保存する
              </button>
            </div>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: "28px 32px", maxWidth: 340, width: "90%", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setDeleteConfirm(null)}
                style={{ flex: 1, background: "#f3f4f6", border: "none", borderRadius: 10, padding: "10px", fontWeight: 600, cursor: "pointer" }}>
                キャンセル
              </button>
              <button onClick={() => handleDelete(deleteConfirm)}
                style={{ flex: 1, background: "#ef4444", color: "#fff", border: "none", borderRadius: 10, padding: "10px", fontWeight: 700, cursor: "pointer" }}>
                削除する
              </button>
            </div>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 20, maxWidth: 520, width: "100%", height: "calc(100vh - 32px)", maxHeight: 600, display: "flex", flexDirection: "column", boxShadow: "0 8px 48px #0004" }}
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "0 24px" }}>
              <div style={{ display: "grid", gap: 16, paddingBottom: 8 }}>
                <Field label="件名 *">
                  <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="例: アイコン用イラスト" style={inp} />
                </Field>
                <Field label="絵師名 *">
                  <input value={form.artist} onChange={e => setForm({ ...form, artist: e.target.value })} placeholder="例: 花咲りん" style={inp} />
                </Field>
                <Field label="X ID（任意）">
                  <input value={form.x_id} onChange={e => setForm({ ...form, x_id: e.target.value })} placeholder="例: @artist_name" style={inp} />
                </Field>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(80px,1fr))", gap: 8, marginBottom: 10 }}>
                          {pendingImages.map(pi => (
                            <div key={pi.id} style={{ position: "relative", borderRadius: 10, overflow: "hidden", border: "1.5px solid #c4b5fd", background: "#f3f4f6" }}>
                              ...
                              <button type="button"
                                onClick={() => { URL.revokeObjectURL(pi.previewUrl); setPendingImages(prev => prev.filter(x => x.id !== pi.id)); }}
                                style={{ position: "absolute", top: 3, right: 3, background: "#ef4444cc", color: "#fff", border: "none", borderRadius: "50%", width: 18, height: 18, fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                ×
                              </button>
                            </div>
                          ))}
                        ...
                        <label style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 14px", border: `1.5px dashed ${atLimit ? "#fca5a5" : "#c4b5fd"}`, borderRadius: 10, cursor: atLimit ? "not-allowed" : "pointer", fontSize: 13, color: atLimit ? "#ef4444" : "#7c3aed", fontWeight: 600, background: atLimit ? "#f3f4f6" : "#fff" }}>
                          ...
                          <input type="file" accept="image/*" disabled={atLimit} style={{ display: "none" }}
                            onChange={e => {
                              ...
                              setPendingImages(prev => [...prev, { id: `${Date.now()}-${Math.random()}`, file, imageType: pendingImageType, previewUrl }]);
                              e.target.value = "";
                            }} />
                        </label>
                      </div>
